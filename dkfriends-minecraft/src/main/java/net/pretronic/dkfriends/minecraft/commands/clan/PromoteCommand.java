    ...
    public void execute(CommandSender commandSender, String[] args) {
        ...
        if(CommandUtil.isInClanCheck(commandSender, player)) {
            ...
            if(targetMinecraftPlayer != null) {
                ...
                if(targetClanMember.canPromote(member)) {
                    targetClanMember.promote(member);
                    commandSender.sendMessage(Messages.COMMAND_CLAN_PROMOTE, VariableSet.create().addDescribed("member", targetClanMember));
                } else {
